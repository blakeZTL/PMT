...
export class AssignedSmeService {
  ...
  private _options = `?$select=pmt_assignedsmeid,pmt_email,pmt_fullname&$expand=pmt_Facility($select=pmt_facilityid,pmt_pmtfacilityid),pmt_smerequest_AssignedSME_pmt_assignedsme($select=pmt_autoid,pmt_smerequestid;$expand=pmt_smehour_Request_pmt_smerequest($select=pmt_autoid,pmt_date,pmt_hours,pmt_smehourid);)`;
